...
class Breadcrumb {
	...
	public static function create_links()
	{
		...
		for ($i=0,$total=static::count(); $i < $total; $i++)
		{
			...
			$output .= ($is_last) ? \Fuel\Core\Inflector::humanize(static::$breadcrumb[$i]["title"], '-') : \Html::anchor(static::$breadcrumb[$i]["link"], \Fuel\Core\Inflector::humanize(static::$breadcrumb[$i]["title"],'-'));
